...
description: "Orchestrate decisions across Kogito rule units, Drools KJAR ruleflows, and BPMN business rule tasks - with sequential, parallel, and conditional patterns over REST and Kafka."
source: "https://aletyx-docs-preview.aletyx.workers.dev/core/integration/decision-process/"
---
...
[<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-1.svg" alt="Flowchart of a classic Drools ruleflow: Start leads to a Validation ruleflow group and a 'Valid Application?' decision; if yes it runs Scoring then Approval Logic, if no it runs Rejection, then End" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-1-dark.svg" alt="Flowchart of a classic Drools ruleflow: Start leads to a Validation ruleflow group and a 'Valid Application?' decision; if yes it runs Scoring then Approval Logic, if no it runs Rejection, then End" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-1.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-2.svg" alt="BPMN flowchart of a hybrid loan process: Start (Loan Application) to a Validate Data service task, then Credit Scoring and Risk Assessment business rule tasks, then an 'Approved?' decision; yes goes to a Final Review user task and End Approved, no goes to End Rejected" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-2-dark.svg" alt="BPMN flowchart of a hybrid loan process: Start (Loan Application) to a Validate Data service task, then Credit Scoring and Risk Assessment business rule tasks, then an 'Approved?' decision; yes goes to a Final Review user task and End Approved, no goes to End Rejected" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-2.svg)
...
See [BPMN Activities](/architecture/processes/basic-bpmn/activities) for detailed configuration guidance.
...
[<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-3.svg" alt="Flowchart of a sequential decision chain: Application Data passes through Validation Rules, Scoring Rules, Pricing Rules and Approval Rules to produce the Decision Output" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-3-dark.svg" alt="Flowchart of a sequential decision chain: Application Data passes through Validation Rules, Scoring Rules, Pricing Rules and Approval Rules to produce the Decision Output" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-lr-3.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-4.svg" alt="Flowchart of parallel decision evaluation: an Application is evaluated simultaneously by Credit Risk Rules, Fraud Detection Rules and Compliance Rules, whose outputs are combined in Aggregate Results to produce the Final Decision" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-4-dark.svg" alt="Flowchart of parallel decision evaluation: an Application is evaluated simultaneously by Credit Risk Rules, Fraud Detection Rules and Compliance Rules, whose outputs are combined in Aggregate Results to produce the Final Decision" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-4.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-5.svg" alt="Flowchart of conditional decision routing: a Product Application is routed by product type to Mortgage, Auto Loan or Credit Card pricing rules, then converges on Generate Quote" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-5-dark.svg" alt="Flowchart of conditional decision routing: a Product Application is routed by product type to Mortgage, Auto Loan or Credit Card pricing rules, then converges on Generate Quote" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/decision-process/flowchart-td-5.svg)
...
See [Event-Driven Integration](/architecture/integration/event-driven) for comprehensive event patterns.
...
See [Service Orchestration](/architecture/integration/service-orchestration) for service task patterns.
...
- **Learn Rule Units:** [Understanding Rule Units](/drools/drl/rule-units)
- **Explore Ruleflows:** [Drools Ruleflow Guide](https://docs.aletyx.ai/drools/ruleflow/)
- **Master BPMN:** [BPMN Activities](/architecture/processes/basic-bpmn/activities)
- **Event Integration:** [Event-Driven Architecture](/architecture/integration/event-driven)
