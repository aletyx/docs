...
[![Sequence diagram of outgoing fraud alerts: the Banking system publishes a Transaction event to Kafka; the Rules service consumes it, evaluates rules, detects a large amount and publishes a FraudAlert; the Alert service consumes it and notifies Operations](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-1.svg)](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-1.svg)
...
[![Sequence diagram of request and response decisioning over Kafka: App publishes a DecisionRequest to the request topic; the DMN service consumes it, evaluates a decision table and publishes a DecisionResponse to the response topic; the Process consumes it and continues the workflow](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-2.svg)](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-2.svg)
...
[![Sequence diagram of complex event choreography over Kafka among Portal, Loan Process, Credit, Fraud and Notification services: ApplicationSubmitted starts the process, which requests and catches CreditCheckComplete and FraudCheckComplete events before making the decision and notifying](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-6.svg)](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-6.svg)
...
[![Flowchart of a saga pattern: Order Created leads to Reserve Inventory and a 'Success?' check; if yes it charges payment and ships the order, if no it compensates by releasing inventory and cancelling the order](/images/architecture/assets/diagrams/core/integration/event-driven/flowchart-td-7.svg)](/images/architecture/assets/diagrams/core/integration/event-driven/flowchart-td-7.svg)
