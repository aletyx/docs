---
title: "Event-Driven"
description: "Build event-driven rules, DMN decisions, and jBPM processes in Kogito using native Kafka and CloudEvents integration, message and catch events, and saga choreography."
source: "https://aletyx-docs-preview.aletyx.workers.dev/core/integration/event-driven/"
---
...
[<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-1.svg" alt="Sequence diagram of outgoing fraud alerts: the Banking system publishes a Transaction event to Kafka; the Rules service consumes it, evaluates rules, detects a large amount and publishes a FraudAlert; the Alert service consumes it and notifies Operations" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-1-dark.svg" alt="Sequence diagram of outgoing fraud alerts: the Banking system publishes a Transaction event to Kafka; the Rules service consumes it, evaluates rules, detects a large amount and publishes a FraudAlert; the Alert service consumes it and notifies Operations" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-1.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-2.svg" alt="Sequence diagram of request and response decisioning over Kafka: App publishes a DecisionRequest to the request topic; the DMN service consumes it, evaluates a decision table and publishes a DecisionResponse to the response topic; the Process consumes it and continues the workflow" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-2-dark.svg" alt="Sequence diagram of request and response decisioning over Kafka: App publishes a DecisionRequest to the request topic; the DMN service consumes it, evaluates a decision table and publishes a DecisionResponse to the response topic; the Process consumes it and continues the workflow" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-2.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-6.svg" alt="Sequence diagram of complex event choreography over Kafka among Portal, Loan Process, Credit, Fraud and Notification services: ApplicationSubmitted starts the process, which requests and catches CreditCheckComplete and FraudCheckComplete events before making the decision and notifying" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/event-driven/sequence-6-dark.svg" alt="Sequence diagram of complex event choreography over Kafka among Portal, Loan Process, Credit, Fraud and Notification services: ApplicationSubmitted starts the process, which requests and catches CreditCheckComplete and FraudCheckComplete events before making the decision and notifying" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/event-driven/sequence-6.svg)
...
[<img src="/images/architecture/assets/diagrams/core/integration/event-driven/flowchart-td-7.svg" alt="Flowchart of a saga pattern: Order Created leads to Reserve Inventory and a 'Success?' check; if yes it charges payment and ships the order, if no it compensates by releasing inventory and cancelling the order" className="block dark:hidden" />
<img src="/images/architecture/assets/diagrams/core/integration/event-driven/flowchart-td-7-dark.svg" alt="Flowchart of a saga pattern: Order Created leads to Reserve Inventory and a 'Success?' check; if yes it charges payment and ships the order, if no it compensates by releasing inventory and cancelling the order" className="hidden dark:block" />](/images/architecture/assets/diagrams/core/integration/event-driven/flowchart-td-7.svg)
...
- **Learn BPMN Events:** [Intermediate Events](/architecture/processes/basic-bpmn/intermediate-events)
- **Explore Processes:** [Advanced BPMN](/architecture/processes/advanced-bpmn/event-driven)
- **Decision Orchestration:** [Decision Process Integration](/architecture/integration/decision-process)
- **Service Tasks:** [Service Orchestration](/architecture/integration/service-orchestration)
