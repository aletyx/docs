...
1. Set the Git variables on the Sidecar (see [Configuration](/decision-control-tower-sidecar/deployment/configuration)):
    - `ALETYX_CONTROL_TOWER_SIDECAR_GITHUB_ENABLED=true`
    - `ALETYX_CONTROL_TOWER_SIDECAR_GITHUB_TOKEN=<token>`
    - `ALETYX_CONTROL_TOWER_SIDECAR_GITHUB_REPO=<owner/repo>`
    - `ALETYX_CONTROL_TOWER_SIDECAR_GITHUB_BRANCH=<branch>` (defaults to `main`)
...
1. Set `ALETYX_CONTROL_TOWER_SIDECAR_GITHUB_ENABLED=false` (or leave it unset).
