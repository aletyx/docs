...
To add a user with the `kie-server` role or the `rest-all` role or both (assuming a Keystore is already set), navigate to `~/$SERVER_HOME` and run the following command with the role or roles specified:

```bash
./bin/add-user.sh -a -u <USERNAME> -p <PASSWORD> -g rest-all,kie-server
```

The `-a` flag adds the user to the application realm (`ApplicationRealm`), and `-g` assigns its groups (comma-separated).
