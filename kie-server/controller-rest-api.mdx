...
To add a user with the `kie-server` role or the `rest-all` role or both, navigate to `~/$SERVER_HOME` and run the following command with the role or roles specified:

```bash
./bin/add-user.sh -a -u <USERNAME> -p <PASSWORD> -g kie-server,rest-all
```
