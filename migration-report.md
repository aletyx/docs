...
## SVG diagram label fix (post-migration)

The d2-generated diagram SVGs copied from the source repo keep their shape labels in `<foreignObject>` HTML. Mintlify's image CDN strips `foreignObject` from SVGs as a security measure ([documented](https://www.mintlify.com/docs/create/image-embeds#svg-images)), so labels disappeared on the deployed site even though the repo files are byte-identical to the originals served by the old site. Fixed by mechanically converting every label to a native SVG `<text>` element at the same coordinates, using the same embedded d2 fonts and colors — 803 labels across 88 SVGs (Decision Control Tower, Decision Control, use cases, and pre-existing architecture/components/deployment diagrams). Script: `scripts/svg-foreignobject-to-text.py` — run it on any diagram SVG copied from the aletyx-docs repo in the future.

## Dark-mode diagram variants (post-migration)

The old site's `svg-fix.js` swapped every SVG image to its `-dark.svg` sibling in dark mode — the markdown itself only ever references the light file, so migrated pages showed the light (white-canvas) render in both themes. Reproduced with Mintlify's standard pattern: copied all 48 missing `-dark.svg` originals from the source repo (label-converted like the rest) and rewrote every affected image reference into a light/dark pair (`className="block dark:hidden"` / `"hidden dark:block"`) — 22 pages, 64 image pairs, including pre-existing pages (integration patterns, Drools visual guide, Phreak, process versioning, MCP, JVM-vs-native, older use cases, quickstart, FAQ).
