...
A managed instance is one that requires an jBPM controller to be available to properly start up the Kie Server instance.

The jBPM controller is a component responsible for keeping and managing a Kie Server Configuration in centralized way.
...
jBPM controllers they should be kept in sync to make sure that regardless
...
any jBPM controllers, please see "Unmanaged Kie Server".
</Note>
...
They are always considered remote to jBPM controller.
The jBPM controller is responsible for persisting the configuration to preserve restarts of the jBPM controller itself.
...
In the above diagram we can see that the Kie Server instances are capable of connecting to any jBPM controllers, but they will connect to only one.
Each instance will attempt to connect to jBPM controller as long as it can reach one.
Once connection is established with one of the jBPM controllers it will skip the others.
...
In most use cases, the Kie Server should be executed in managed mode as that provides some benefits, like a web user interface (if using Business Central as an jBPM controller) and some facilities for clustering.
