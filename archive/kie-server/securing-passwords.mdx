...
description: "Store KIE Server passwords in a key store instead of clear text on the disc and use them in the application."
...
2. use key tool to create keystore with password in it
...
3. set following system properties on application server that will let the KIE Server or jBPM controller to read password from keystore
...
4. start server to verify configuration
