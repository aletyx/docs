...
description: "API reference documentation, authentication patterns, integration examples, and error handling guidance for integrating Decision Control into your enterprise architecture."
source: "https://aletyx-docs-preview.aletyx.workers.dev/decision-control/integration-and-apis/"
---
...
Decision Control provides two API surfaces:
...
Governance workflows — approvals, four-eyes enforcement, and audit trails — are managed through Aletyx Decision Control Tower (Keystone Edition). See the [Aletyx Decision Control Tower documentation](/decision-control-tower/overview) for governance features.
...
| `400 Bad Request` | Invalid request data | Missing required fields, invalid JSON, malformed DMN payload |
| `401 Unauthorized` | Authentication required | Missing or invalid access token |
| `403 Forbidden` | Operation not permitted | Free-tier model limit exceeded, AI Assistant not configured |
| `404 Not Found` | Resource not found | Invalid unit / version / model ID |
| `409 Conflict` | Resource conflict | Duplicate unit name; deletion blocked by existing versions |
| `500 Internal Server Error` | Unexpected server error | Database connectivity, unhandled runtime failure |
...
```json
{ "error": "Unit name 'financial-services' already exists" }
```
...
- [Usage Scenarios](/decision-control/usage-scenarios): Common integration patterns and workflows
- [Architecture](/decision-control/architecture): Deep dive into system design
