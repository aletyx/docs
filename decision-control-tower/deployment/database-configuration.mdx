...
For deployments that must use a non-default database name, override `SPRING_DATASOURCE_URL`:
...
| ----------------------------- | ---------------- | ---------------------------------------------------- | -------------------------------------------------------------- |
| `SPRING_DATASOURCE_URL`      | yes (production) | `jdbc:postgresql://localhost:5434/decision_tower_db` | JDBC connection URL                                            |
| `SPRING_DATASOURCE_USERNAME` | yes (production) | `tower`                                              | Database username                                              |
| `SPRING_DATASOURCE_PASSWORD` | yes (production) | `tower`                                              | Database password (provide via secret manager — see chapter 3) |
...
services:
  ...
  decision-control-tower:
    ...
    environment:
      SPRING_DATASOURCE_URL: jdbc:postgresql://tower-database:5432/decision_tower_db
