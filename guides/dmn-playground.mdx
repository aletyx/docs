...
<Steps>
  ...
  <Step title="Review the model">
    ...
    <Accordion title="What each part of the DMN editor does">
      **Tabs and top controls**

      - **Editor tab** — The active view, showing the Decision Requirements Diagram (DRG/DRD) or your current selection.
      - **Data Types tab** — View, create, or edit the data types associated with the model.
      - **Included Models tab** — Reference other models so decisions, data types, and logic can be reused.
      - **DRG/DRD selector** (dropdown at the top-left of the canvas) — Navigate between the diagram's nodes.
      - **Decision Output Type** (dropdown above a selected node) — Map the decision's data type to a particular part of your model or structures.
      - **Properties panel** (the *i* button, top-right — or press `i` with a node selected) — Open the selected node's properties.

      **Diagram nodes**

      - **Decision node** (rectangle, e.g. "Credit Score Rating") — Places a decision on the diagram. A decision with no inputs executes on invocation; if it has an incoming requirement (another decision or an input node), that must complete first before the decision executes.
      - **Input Data node** (oval, e.g. "Credit Score") — Data required to execute a decision. Can be mapped to specific data structures or types.
      - **Business Knowledge Model (BKM)** — Reusable decision logic written in Friendly Enough Expression Language (FEEL), Java, or PMML.
      ...
      - **Annotation** — Free-text notes on the model; no executable impact, but great for explaining a model.

      **Connectors and per-node actions**

      - **Connectors** — Drag from a node to connect it to the appropriate target — another decision, an input, or a knowledge source.
      - **Edit** (below a selected node) — Open the node's decision logic.
    </Accordion>
  ...
  <Step title="Inspect the decision table">
    ...
    <Accordion title="What each part of the decision table does">
      **Columns**
      ...
      - **Input column / condition** (e.g. "Credit Score.FICO (number)") — Sets the condition on the FICO score. The dot notation takes the `FICO` property from the input data. The rule rows below use the supplied score to determine the rating.
      - **Output column** (e.g. "Credit Score Rating (Credit_Score_Rating)") — Sets the decision's output value when a row's conditions match. Here it maps to the `Credit_Score_Rating` data type.
      - **Annotations column** (optional) — Notes about each rule; ignored by the engine during evaluation.

      **Cell expressions**

      - **FEEL condition** — Comparison expressions are common. Use brackets `[]` (inclusive) or parentheses `()` (exclusive) with `..` to write ranges. Note the Unique (U) hit policy requires that only a single rule can fire across the table.
      - **FEEL output** — The value set when the rule's conditions are met. Strings use quotes (as shown), but this could just as easily be a number or a FEEL expression.
